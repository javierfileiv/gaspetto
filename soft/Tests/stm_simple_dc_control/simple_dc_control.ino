...
const uint32_t channel_M2_FWD = STM_PIN_CHANNEL(
...
TIM_TypeDef *Timer2 = (TIM_TypeDef *)pinmap_peripheral(
    digitalPinToPinName(pin_M1_FWD), PinMap_PWM);
HardwareTimer *m1_timer = new HardwareTimer(Timer2);

TIM_TypeDef *Timer3 = (TIM_TypeDef *)pinmap_peripheral(
    digitalPinToPinName(pin_M2_FWD), PinMap_PWM);
...
void m1_speed_irq(void) { ++m1_counter; }

void m2_speed_irq(void) { ++m2_counter; }
...
static void setMotor(enum motor motor, enum direction dir, uint32_t speed) {
  ...
  if (motor == MOTOR_1) {
    switch (dir) {
    case FORDWARD:
      m1_timer->setCaptureCompare(channel_M1_FWD, speed,
                                  PERCENT_COMPARE_FORMAT);
      break;
    case BACKWARD:
      m1_timer->setCaptureCompare(channel_M1_BWD, speed,
                                  PERCENT_COMPARE_FORMAT);
      break;
    }
  } else {
    switch (dir) {
    case FORDWARD:
      m2_timer->setCaptureCompare(channel_M2_FWD, speed,
                                  PERCENT_COMPARE_FORMAT);
      break;
    case BACKWARD:
      m2_timer->setCaptureCompare(channel_M2_BWD, speed,
                                  PERCENT_COMPARE_FORMAT);
      break;
    }
  }
