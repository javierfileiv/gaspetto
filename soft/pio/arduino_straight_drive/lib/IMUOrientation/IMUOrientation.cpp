...
void IMUOrientation::calibrate(bool print) {
    // Warm up
    for(int i=0;i<50;i++){ sensors_event_t a,g,temp; mpu.getEvent(&a,&g,&temp); delay(5);}
    const int N=600;
    ...
    if(print){
        ...
        Serial.print(F("Acc Offsets: ")); Serial.print(offsets.accX,4); Serial.print(','); Serial.print(offsets.accY,4); Serial.print(','); Serial.println(offsets.accZ,4);
        Serial.print(F("Gyro Offsets: ")); Serial.print(offsets.gyroX,4); Serial.print(','); Serial.print(offsets.gyroY,4); Serial.print(','); Serial.println(offsets.gyroZ,4);
    }
    // Initialize orientation from accel
    sensors_event_t a2,g2,t2; mpu.getEvent(&a2,&g2,&t2);
    rollDeg  = atan2(a2.acceleration.y, a2.acceleration.z) * RAD_TO_DEG;
    pitchDeg = atan2(-a2.acceleration.x, sqrt(a2.acceleration.y*a2.acceleration.y + a2.acceleration.z*a2.acceleration.z)) * RAD_TO_DEG;
    yawDeg = 0.0f;
}
