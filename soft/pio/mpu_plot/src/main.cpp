...
float kalmanFilter(float newAngle, float newRate, float &currentAngle, float &currentBias,
                   float &P00, float &P01, float &P10, float &P11, float dt);
...
void setup()
{
    ...
    kalman_pitch_angle =
        fast_atan2f(-a.acceleration.x, fast_sqrtf(a.acceleration.y * a.acceleration.y +
    ...
    Serial1.println(
        "Streaming Seq,Micros,AccX,AccY,AccZ,GyroX,GyroY,GyroZ,KalmanRoll,KalmanPitch,IntegratedYaw");
}
...
void calibrateMPU6050()
{
    ...
    Serial1.print("Acc Offsets (m/s^2): X=");
    Serial1.print(accOffset_X);
    Serial1.print(", Y=");
    Serial1.print(accOffset_Y);
    Serial1.print(", Z=");
    Serial1.println(accOffset_Z);
    Serial1.print("Gyro Offsets (rad/s): X=");
    Serial1.print(gyroOffset_X);
    Serial1.print(", Y=");
    Serial1.print(gyroOffset_Y);
    Serial1.print(", Z=");
    Serial1.println(gyroOffset_Z);
...
void loop()
{
    ...
    kalman_roll_angle  = kalmanFilter(accel_roll, g.gyro.x * 180 / PI, kalman_roll_angle,
                                      kalman_roll_bias, P_00_roll, P_01_roll, P_10_roll, P_11_roll,
                                      dt);
    kalman_pitch_angle = kalmanFilter(accel_pitch, g.gyro.y * 180 / PI, kalman_pitch_angle,
                                      kalman_pitch_bias, P_00_pitch, P_01_pitch, P_10_pitch,
                                      P_11_pitch, dt);
    ...
    while (integratedYaw > 180)
        integratedYaw -= 360;
    while (integratedYaw < -180)
        integratedYaw += 360;

    // Print CSV line:
    // Seq,Micros,AccX,AccY,AccZ,GyroX,GyroY,GyroZ,KalmanRoll,KalmanPitch,IntegratedYaw
    Serial1.print(seqCounter);
    Serial1.print(',');
    Serial1.print(currentMicros);
    Serial1.print(',');
    Serial1.print(a.acceleration.x, 6);
    Serial1.print(',');
    Serial1.print(a.acceleration.y, 6);
    Serial1.print(',');
    Serial1.print(a.acceleration.z, 6);
    Serial1.print(',');
    Serial1.print(g.gyro.x, 6);
    Serial1.print(',');
    Serial1.print(g.gyro.y, 6);
    Serial1.print(',');
    Serial1.print(g.gyro.z, 6);
    Serial1.print(',');
    Serial1.print(kalman_roll_angle, 4);
    Serial1.print(',');
    Serial1.print(kalman_pitch_angle, 4);
    Serial1.print(',');
